...
def list_player_quests(player_id: str, db_path: Path | None = None) -> list[dict]:
    with connect(db_path) as connection:
        rows = connection.execute(
            """
            SELECT id, player_id, template_id, run_id, offered_by_npc_id,
...
def load_player_quest(player_id: str, quest_id: str, db_path: Path | None = None) -> dict | None:
    with connect(db_path) as connection:
        row = connection.execute(
            """
            SELECT id, player_id, template_id, run_id, offered_by_npc_id,
