...
class CombatParleyTests(unittest.TestCase):
    ...
    def test_parley_open_uses_service_generated_line(self) -> None:
        resolve_turn(self.world, self.state, "parley_open", parley_service=self.parley)

        negotiation = self.state.combat_state["negotiation"]
    ...
    def test_combat_state_is_turn_based_without_legacy_board_state(self) -> None:
        available_actions = {entry["id"] for entry in self.state.combat_state["available_actions"]}

        self.assertEqual("turn-based", self.state.combat_state["mode"])
        self.assertIn("enemy", self.state.combat_state)
        self.assertIn("available_actions", self.state.combat_state)
