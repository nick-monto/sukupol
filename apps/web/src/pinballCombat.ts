...
const WEAPON_BUMPER_SHAPE: Record<
	string,
...
export function setupPinball(
	...
	// ── Hatch helpers ──────────────────────────────────────────────────────
	function openHatch() {
		Body.translate(hatch, { x: 0, y: cabinet.hatchTranslateY });
		hatchUp = false;
	}
	function closeHatch() {
		Body.translate(hatch, { x: 0, y: -cabinet.hatchTranslateY });
	...
	Events.on(engine, "beforeUpdate", () => {
		...
		if (ball) {
			...
			// Guard only triggers in the shooter-lane return channel: below the hatch
			// opening AND above the flippers. The upper Y bound prevents the guard from
			// firing in the lower-right playfield (side-kicker bounce zone), which was
			// causing ghost bounces when the ball fell downward past the guard X line.
			if (
	...
	function frame(ts: number): void {
		const elapsed =
			lastTs === 0 ? FIXED_DT : Math.min(ts - lastTs, FIXED_DT * 5);
		lastTs = ts;
		...
		ctx.save();
		ctx.globalAlpha = 0.55;
		for (const w of guardrailBodies) drawPoly(w, theme.walls);
		ctx.restore();
		...
		// Weak-point — pulsing ring
		{
			...
			drawCirc(
				...
				wpSpec.y,
				wpSpec.radius,
				wpFlash ? "#ffffff" : theme.weakPoint,
			);
		...
		ctx.beginPath();
		ctx.arc(fl.hingeX, fl.hingeY, 4, 0, Math.PI * 2);
		ctx.fill();
		ctx.beginPath();
		ctx.arc(fr.hingeX, fr.hingeY, 4, 0, Math.PI * 2);
		ctx.fill();

		if (ball)
			drawCirc(ball.position.x, ball.position.y, BALL_RADIUS, theme.ball);
